      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantSection = details.participants.length
          ? `<ul class="participants-list">${details.participants
              .map(
                (participant) =>
                  `<li><span class="participant-email">${participant}</span><button type="button" class="participant-remove-button" data-activity="${name}" data-email="${participant}" aria-label="Remove participant">×</button></li>`
              )
              .join("")}</ul>`
